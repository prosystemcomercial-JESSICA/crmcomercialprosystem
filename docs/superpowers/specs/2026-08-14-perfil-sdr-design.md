...
- **Decisão registrada:** a terceira saída do sensor-mercado (lista de `funcionalidade_solicitada`, texto livre) foi descartada na implementação. Em vez de um campo/lista separada, "funcionalidade ausente" passou a ser tratada como mais uma categoria contável na taxonomia de `motivo_perda` (`FUNCIONALIDADE_AUSENTE`), agregada junto com as outras 6 objeções em vez de listada como texto livre à parte.
