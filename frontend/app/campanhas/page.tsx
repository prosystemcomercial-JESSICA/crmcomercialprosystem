    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-sm font-semibold">Campanhas</h1>
        ...
        {/* Aviso para vendedor */}
        {!podeGerenciar && (
          <div style={{ background: 'var(--t-primary-light)', border: '1px solid #C3DCFC', borderRadius: 10, padding: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 18 }}>📢</span>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--t-primary)' }}>Visualização da equipe comercial</div>
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              { label: 'Total', value: stats.total, color: 'text-gray-700', bg: 'bg-opacity-0' },
          ...
          {['', 'RASCUNHO', 'ATIVA', 'PAUSADA', 'FINALIZADA', 'ARQUIVADA'].map(s => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                statusFilter === s ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200  hover:bg-opacity-0'
              }`}
            >
              {s === '' ? 'Todas' : s}
            </button>
        ...
        {dataLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {campanhas.map(c => (
              <div key={c.id} className="ps-card rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm font-semibold truncate">{c.nome}</h3>
                    {c.descricao && <p className="text-sm  mt-0.5 line-clamp-2">{c.descricao}</p>}
                  </div>
                ...
                {podeGerenciar ? (
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
                    <span className={`text-xs font-semibold px-3 py-1 rounded-full ${STATUS_COLORS[c.status] || 'bg-opacity-0 '}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="ps-card rounded-xl shadow-xl p-6 w-full max-w-lg mx-4">
            <h2 className="text-xl font-bold text-sm font-semibold mb-5">{editingId ? 'Editar Campanha' : 'Nova Campanha'}</h2>
            ...
            <div className="flex gap-3 mt-6 justify-end">
              <button onClick={() => setShowModal(false)} className="px-4 py-2  hover:opacity-80 rounded-lg transition-colors">Cancelar</button>
