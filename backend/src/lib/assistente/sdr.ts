...
export function promptCaroline(p: {
  ...
  const sistema = [
    ...
    p.atualidades?.length ? '\n=== ASSUNTOS DA SEMANA (pesquisa da Sofia, com fonte; use no máximo UM, só se tiver a ver com o negócio do lead, sem inventar detalhes além do que está aqui) ===\n' + p.atualidades.map(a => `- [${a.segmento}] ${a.titulo}: ${a.resumo} (por que importa: ${a.por_que_importa})`).join('\n') : '',
    p.instrucoes || '',
  ].join('\n');
  const l = p.lead;
