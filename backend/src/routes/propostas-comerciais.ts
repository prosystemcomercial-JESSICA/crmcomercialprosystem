...
export async function propostasComerciais(fastify: FastifyInstance, options: { prisma: PrismaClient }) {
  ...
  fastify.post('/propostas-comerciais', async (request, reply) => {
    ...
    // (Antes havia aqui um desvio da conta mock 'user-jessica' para a conta
    // "Jessica Vendedora". Com a unificação, a Jessica vende pela própria conta:
    // sem vendedor_id explícito, a venda é de quem está logado.)
