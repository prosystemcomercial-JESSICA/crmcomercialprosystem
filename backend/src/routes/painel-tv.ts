...
async function lerConfigTv(prisma: PrismaClient, chaves: string[]) {
...
async function lerMetasTv(prisma: PrismaClient) {
  const cfg = await lerConfigTv(prisma, Object.values(CHAVES_METAS_TV));
...
/** Números do painel (tela 1 = hoje, tela 2 = ano). Usado pela TV e pelo resumo executivo por e-mail. */
export async function montarDadosPainelTv(prisma: PrismaClient) {
  const L = limitesPeriodo(new Date());
  const { agora, inicioHoje, fimHoje, inicioMes, fimMes, inicioAno, fimAno } = L;
  const metas = await lerMetasTv(prisma);
...
export async function painelTvRoutes(fastify: FastifyInstance, options: { prisma: PrismaClient }) {
  ...
  const lerConfig = (chaves: string[]) => lerConfigTv(prisma, chaves);
  const lerMetas = () => lerMetasTv(prisma);
